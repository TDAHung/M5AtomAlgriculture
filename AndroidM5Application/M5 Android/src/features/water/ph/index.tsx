...
const PH = () => {
    ...
    if (Number(range.max_normal) != -1 && Number(range.min_normal != -1)) {
        maxPH = Number(range.max_normal);
        minPH = Number(range.min_normal);
    }
    ...
    useEffect(() => {
        const firebaseConnection = () => {
            firestore().collection('water_ph_state').onSnapshot(data => {
                data.forEach((item) => {
                    let ratio;
                    if (item.data().value > maxPH) ratio = 1;
                    else if (item.data().value < minPH) ratio = 0;
                    else ratio = (item.data().value - minPH) / (maxPH - minPH);
                    setPHState([
                        ratio
                    ]);
                });
            ...
            firestore().collection('water_ph').onSnapshot(data => {
                const dataFetch: any = [];
                data.forEach((item) => {
