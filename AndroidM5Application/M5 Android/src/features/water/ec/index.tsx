    ...
    useEffect(() => {
        const firebaseConnection = () => {
            firestore().collection('water_ec_state').onSnapshot(data => {
                data.forEach((item) => {
                    let ratio;
                    if (item.data().value > maxEC) ratio = 1;
                    else if (item.data().value < minEC) ratio = 0;
                    else ratio = (item.data().value - minEC) / (maxEC - minEC);
                    setECState([
                        ratio
                    ]);
                });
            ...
            firestore().collection('water_ec').onSnapshot(data => {
                const dataFetch: any = [];
                data.forEach((item) => {
            ...
            <View style={{ flex: 1 }}>
                ...
                <View>
                    <View style={styles.chartWrapper} >
                        {
                            renderProgressChart(ecState, ["EC"])
                        }
